...
if __name__ == "__main__": 
	...
	word_list = re.findall(r"\w+[\.-]\w+[\.-]?\w*|\d+,\d+|\d*\.\d+|[\w]+|[-.,!?;:&$()\"]", text)

	paragraph_count = len(re.findall(r"(\n\n\w)", text)) + 1 #RE finds gaps between paragraphs so add 1 to account for first paragraph  
	token_count = {}
	type_count = {} 
	total_count = 0
	...
	sort_dict_by_count = sorted(token_count.items(), key=operator.itemgetter(1), reverse=True)
